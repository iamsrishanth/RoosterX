...
- **signature-dish-bento** - balanced asymmetric grid (Row 1: 2-col featured hero wrap + 1-col specialty card; Row 2: 3 equal category cards): dish photo, name (h3), description, gold price. Real dishes and real prices only; no empty cells or dead space.
...
- Scene: Home hero ONLY - a rising ember/spark particle field (instanced points, additive blending, flame and gold emitters on smoke) behind the hero photograph with soft bloom and pointer parallax.
- Motion: particles rise slowly with gentle horizontal sway; pointer parallax max 3 degrees, desktop only.
- Post: soft bloom (mipmap-blur, calibrated luminance threshold) touching the embers - the food photo and copy stay crisp.
